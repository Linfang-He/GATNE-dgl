...
def generate_walks(network_data, num_walks, walk_length, schema, file_name):
    ...
    all_walks = []
    for layer_id in network_data:  # layer_id: edge_type, only 1 and 2 in example
...
def generate_pairs(all_walks, vocab, window_size):
    ...
    pairs = []
    skip_window = window_size // 2  # 5 // 2 = 2
    for layer_id, walks in enumerate(all_walks):
        for walk in walks:
            for i in range(len(walk)):  # from 0 to 9
                for j in range(1, skip_window + 1):  # from 1 to 2
                    if i - j >= 0:
...
def generate_vocab(all_walks):
    index2word = []
    raw_vocab = defaultdict(int)  # stores the count of a word appearing in the walk
    ...
    for word, v in iteritems(raw_vocab):
        vocab[word] = Vocab(count=v, index=len(index2word))  # vocab[word] = (count, index)
        index2word.append(word)  # index2word[index] == word

    index2word.sort(key=lambda word: vocab[word].count, reverse=True)  # decending order
